...
| `tf4-cdo07-app-sg` | 8080 từ `tf4-cdo07-alb-sg` only | 443 → VPC Endpoints & AWS API (Kinesis, CloudWatch, ECR) | Mock Services, AI Engine (ECS Fargate) |
| `tf4-cdo07-lambda-sg` | (none — triggered by Kinesis event source mapping, AWS-internal) | 443 → VPC Endpoints (Timestream, S3, SSM, KMS, CloudWatch) | Lambda Transformer, Lambda Window Feeder, Lambda CB |
| `tf4-cdo07-vpce-sg` | 443 từ `tf4-cdo07-app-sg` & `tf4-cdo07-lambda-sg` | (none) | Tất cả Interface VPC Endpoints |
...
Để bảo vệ hệ thống trước các cuộc tấn công DDoS và lạm dụng API, hệ thống tích hợp AWS WAF trực tiếp trên Application Load Balancer (ALB), kết hợp cơ chế PII & Ingestion Firewall tại tầng xử lý sự kiện:

| Thành phần | Cơ chế bảo vệ | Action |
|---|---|---|
| **AWS WAF** | Cấu hình giới hạn tần suất cuộc gọi (Rate Limiting) ở mức 10k req/5min per source IP trên ALB | Rate Limiting |
...
|---|---|---|---|
| `tf4-cdo07-ai-engine-task-role` | AI Engine (ECS Fargate) | `s3:PutObject` (audit bucket `tf4-cdo07-audit-log`), `s3:GetObject` (baseline bucket `tf4-cdo07-baseline-models`), `timestream:Select`, `timestream:DescribeEndpoints` (query 2h metrics window), `cloudwatch:PutMetricData`, `logs:PutLogEvents`, `kms:GenerateDataKey`, `kms:Decrypt` (CMK ARN) | `iam:*`, `s3:Delete*`, `ec2:*`, `timestream:WriteRecords` |
| `tf4-cdo07-mock-svc-task-role` | Mock Services (ECS Fargate) | `kinesis:PutRecord`, `kinesis:PutRecords` (KDS Stream ARN), `cloudwatch:PutMetricData`, `logs:PutLogEvents`, `kms:GenerateDataKey` (encrypt stream records) | `timestream:*`, `s3:*`, `iam:*` |
| `tf4-cdo07-lambda-transformer-role` | Lambda Transformer | `kinesis:GetRecords`, `kinesis:GetShardIterator`, `kinesis:DescribeStream` (from KDS), `timestream:WriteRecords` (TSDB ARN), `timestream:DescribeEndpoints`, `kms:Decrypt` (decrypt Kinesis records), `logs:PutLogEvents` | `s3:*`, `iam:*` |
| `tf4-cdo07-lambda-feeder-role` | Lambda Window Feeder | `timestream:Select` (query 2h metrics), `timestream:DescribeEndpoints`, `ssm:GetParameter` (check `InferenceEnabled`), `sns:Publish` (scoped ARN: `arn:aws:sns:ap-southeast-1:<ACCOUNT>:tf4-cdo07-alerts`), `kms:Decrypt` (decrypt parameters), `logs:PutLogEvents` | `timestream:WriteRecords`, `s3:*`, `iam:*` |
| `tf4-cdo07-lambda-cb-role` | Lambda CB (Cost CB) | `ssm:PutParameter` (update `InferenceEnabled` parameter), `kms:GenerateDataKey`, `kms:Decrypt`, `logs:PutLogEvents` | `s3:*`, `timestream:*`, `iam:*` |
| `tf4-cdo07-lambda-fallback-role` | Lambda Fail-Open Fallback | `sns:Publish` (scoped ARN Topic alerts), `logs:PutLogEvents` | `iam:*`, `s3:*`, `timestream:*` |
| `tf4-cdo07-eventbridge-invoke-role` | EventBridge (1-min schedule) | `lambda:InvokeFunction` (scoped ARN: `arn:aws:lambda:ap-southeast-1:<ACCOUNT>:function:tf4-cdo07-window-feeder`) | `iam:*`, `s3:*`, `timestream:*`, `kms:*` |
...
> **Tại sao OIDC?** Loại bỏ các static credentials (AWS_ACCESS_KEY_ID / SECRET) khỏi GitHub Secrets. Token tự hết hạn sau 1 giờ, làm giảm blast radius nếu CI runner bị compromise.

### 2.3 Permission Boundary

- Boundary ARN: `arn:aws:iam::<ACCOUNT>:policy/tf4-cdo07-boundary`
- Thực thi: Không cho phép bất kỳ role nào tạo bởi `tf4-cdo07-platform-deploy-role` có quyền vượt ra ngoài `tf4-cdo07-*` resource scope.
...
- **K8s RBAC**: Không áp dụng — project dùng ECS Fargate, không dùng EKS (xem [ADR-003](file:///E:/Work/Developer/AWS/XBrain_devop_cloud/ThucHanh/CDO-07-Capstone-phase2/xbrain-learners/08_adrs.md#adr-003---compute-cho-ai-serving-ecs-fargate-over-lambda-eks)).

### 2.6 Multi-tenant Security Isolation (Storage & Ingestion Stream Layers)
...
### 2.7 Tenant Onboarding Security Flow (IAM Provisioning & NFR Mapping)
...
1.  **Xác thực OIDC & Khởi động pipeline (Thời gian: ~1 phút)**:
    *   Admin kích hoạt script `./scripts/tenant-onboard.sh`. Pipeline sử dụng kết nối OIDC để assume role `tf4-cdo07-platform-deploy-role` có hiệu lực ngắn (1 giờ), đảm bảo không dùng static credentials.
2.  **Khởi tạo Tài nguyên Hạ tầng & IAM Provisioning (Thời gian: ~2 phút)**:
    *   **SSM Parameter Registration**: Vì sử dụng kiến trúc Pooled Compute, ta không cần tạo mới các IAM Role cho mỗi tenant mới (loại bỏ hoàn toàn độ trễ do IAM propagation delay). Thay vào đó, Terraform chỉ cần ghi nhận metadata của tenant mới vào SSM Parameter Store dưới đường dẫn `/tf4/cdo07/tenants/{tenant_id}`.
    *   **KMS Key Grants**: Khởi tạo KMS Grant liên kết với CMK của dự án để cấp quyền giải mã/mã hóa dữ liệu cho luồng metric và config của tenant mới.
3.  **Cấu hình Giám sát Grafana (Thời gian: ~1 phút)**:
    *   Pipeline gọi API của Amazon Managed Grafana để tự động tạo mới một Workspace Datasource được cấu hình sẵn câu lệnh lọc mặc định theo `tenant_id` của tenant mới, đồng thời import mẫu Dashboard chuẩn của dự án.
...
5.  **Kết quả**: Tổng thời gian thực tế để hoàn thành onboarding một tenant mới chỉ khoảng **5 phút**, hoàn toàn vượt trội so với yêu cầu NFR < 30 phút của dự án.

---

## 3. Secrets & Configuration Management

### 3.1 Configuration & Parameter Inventory

Kiến trúc mới không sử dụng Secrets Manager cho các secret động. Toàn bộ cấu hình hệ thống (nhập vào qua Terraform hoặc thay đổi động ở runtime) được quản lý tập trung và bảo mật bên trong **SSM Parameter Store**:

| Parameter Path | Dữ liệu lưu trữ | Cơ chế bảo vệ | Accessed by |
|---|---|---|---|
| `/tf4/cdo07/InferenceEnabled` | Trạng thái cho phép/ngăn chặn gọi AI Predict (`true`/`false`) | SecureString (KMS CMK) | `Lambda Window Feeder` (Read), `Lambda CB` (Write) |
| `/tf4/cdo07/tenants/{tenant_id}` | Metadata và schema cấu hình của từng tenant | String | `Lambda Transformer` (Read), `Lambda Window Feeder` (Read) |

**Không áp dụng trong project này (Secrets Manager):**
...
| DB credentials (RDS) | Database là Timestream serverless và Kinesis — sử dụng IAM Auth, không cần credentials. |
| Slack webhook URL | Thông báo đẩy từ SNS trực tiếp sang Slack qua Chatbot/Integration, không expose webhook key tại runtime. |

### 3.2 Parameter Inject Pattern

*   **AWS Lambda & ECS Tasks**: Cấu hình và parameter nhạy cảm được nạp động thông qua SSM Parameter Store bằng cách tích hợp biến trực tiếp vào mã nguồn ứng dụng (ví dụ: sử dụng AWS SDK để gọi `ssm:GetParameter` hoặc thông qua parameter reference trong task/function definition).

### 3.3 Anti-leak Controls

*   **Gitleaks** scan trong CI pipeline — block merge nếu detect secret pattern.
*   **Pre-commit hook**: `.pre-commit-config.yaml` bao gồm `detect-secrets`.
*   **ECR Image Scanning**: Kích hoạt Amazon ECR image scanning (AWS Inspector) cho Mock Services và AI Engine.
*   **SSM Parameter Encryption**: Toàn bộ tham số được lưu trữ dưới dạng `SecureString` mã hóa bởi CMK.
...
*   **Metric Ingestion Buffer (Kinesis Data Streams)**: Dữ liệu metric lưu tạm thời trong Kinesis Data Streams được mã hóa tĩnh mặc định sử dụng Customer Managed Key (CMK) liên kết với KMS.
*   **Audit Log (AI decisions)**: Được lưu trữ tại Amazon S3 bucket `tf4-cdo07-audit-log` và bắt buộc mã hóa bằng Customer Managed Key (CMK) mã định danh `tf4-cdo07-audit-cmk`. Dữ liệu được tự động chuyển đổi sang S3 Infrequent Access (S3 IA) sau 30 ngày để tối ưu chi phí (nhất quán với thiết kế hạ tầng), chuyển sang Glacier Deep Archive sau 90 ngày phục vụ compliance lưu giữ 1 năm (ADR-005).
...
*   **AI Decision Audit Log**: Lưu trữ trực tiếp tại S3 Audit Bucket. Dữ liệu được lưu trữ nóng tại S3 Standard trong **30 ngày đầu**, sau đó tự động chuyển đổi sang **S3 Infrequent Access (S3 IA)** để tối ưu chi phí (khớp với thiết kế hạ tầng `02_infra_design(2).md`). Để đáp ứng yêu cầu lưu trữ 1 năm phục vụ đối soát tuân thủ (ADR-005), dữ liệu sẽ chuyển tiếp sang lớp lưu trữ lạnh **S3 Glacier Deep Archive** sau 90 ngày trước khi tự động xóa sau 365 ngày.
    *   **Lưu ý về truy xuất từ Glacier (SLA & Operational Implication)**: Dữ liệu audit log sau 90 ngày sẽ được chuyển sang lớp lưu trữ lạnh S3 Glacier Deep Archive để tối ưu hóa chi phí.
        *   *SLA khôi phục*: Thời gian khôi phục dữ liệu (retrieval SLA) từ Glacier Deep Archive dao động từ **12 đến 48 giờ** (đối với phương thức Standard/Bulk).
        *   *Operational Implication (Tác động vận hành khi audit khẩn)*: Trong trường hợp cần thực hiện kiểm toán khẩn cấp (Emergency Audit) đối với các quyết định AI cũ hơn 90 ngày, dữ liệu **không thể truy vấn tức thì** qua Amazon Athena. Đội ngũ vận hành (Ops) phải thực hiện lệnh khôi phục (`RestoreObject`) thông qua AWS CLI/Console, chờ từ 12–48 giờ để dữ liệu được chuyển về lớp S3 Standard tạm thời (với retention tùy chọn, ví dụ: 7 ngày) trước khi chạy các câu lệnh SQL Athena. Điều này đã được ghi nhận trong chính sách vận hành của dự án (tham khảo [ADR-005](file:///E:/Work/Developer/AWS/XBrain_devop_cloud/ThucHanh/CDO-07-Capstone-phase2/xbrain-learners/08_adrs.md#adr-005---audit-log-storage-amazon-s3-lifecycle-policy-over-dynamodb)).
...
*   **Application Runtime & Ingest Logs**: Lưu trữ trực tiếp tại Amazon CloudWatch Logs nhằm phục vụ gỡ lỗi trực tiếp. Thời gian duy trì là 14 ngày đối với log ứng dụng thông thường và 7 ngày đối với log hoạt động của luồng ingest data.
...
> **Trạng thái**: N/A — ECS Fargate (Không sử dụng Amazon EKS / Kubernetes) · Xem [ADR-003](file:///E:/Work/Developer/AWS/XBrain_devop_cloud/ThucHanh/CDO-07-Capstone-phase2/xbrain-learners/08_adrs.md#adr-003---compute-cho-ai-serving-ecs-fargate-over-lambda-eks) để biết chi tiết về quyết định lựa chọn Compute Target.

Kiến trúc hệ thống sử dụng dịch vụ chạy container serverless **Amazon ECS Fargate** thay vì tự vận hành cụm Kubernetes (EKS). Do đó, các yêu cầu bảo mật đặc thù của Kubernetes (như K8s RBAC, NetworkPolicies, ServiceAccounts, Pod Security Standards) được đánh giá là **Không áp dụng (N/A)**. Tuy nhiên, an toàn thông tin lớp container vẫn được đảm bảo thông qua các cơ chế thay thế tương đương từ AWS:

*   **Không quản lý nút (No Node Management)**: AWS Fargate chịu trách nhiệm quản lý toàn bộ hệ điều hành của host machine, các bản vá bảo mật hạt nhân (kernel patching), và cô lập phần cứng ở cấp độ hypervisor giữa các ECS task khác nhau. Điều này loại bỏ hoàn toàn rủi ro tấn công leo thang đặc quyền từ container lên node host.
*   **Cô lập mạng cấp Task (Task-level Network Isolation)**: Mỗi ECS Task được gán một Elastic Network Interface (ENI) riêng biệt nằm trong subnet private của VPC. Các quy tắc bảo mật (Security Groups) được áp dụng trực tiếp lên từng Task (ví dụ: `tf4-cdo07-app-sg`), đóng vai trò tương đương như NetworkPolicies trong Kubernetes để kiểm soát chặt chẽ inbound/outbound ports.
*   **Quản lý đặc quyền tối thiểu qua IAM Task Role**: ECS sử dụng cơ chế gán trực tiếp IAM Task Role cho từng container definition. Cơ chế này đơn giản hóa việc phân quyền đặc quyền tối thiểu và giảm thiểu rủi ro cấu hình sai.
*   **Quét lỗ hổng ECR Image (Container Vulnerability Scanning)**: Kích hoạt Amazon ECR image scanning (kết hợp AWS Inspector). Pipeline CI/CD sẽ tự động chặn triển khai đối với các container image chứa lỗ hổng bảo mật mức độ CRITICAL hoặc HIGH.
*   **Bảo vệ logic Fail-Open Fallback (Mảng ứng dụng và cảnh báo)**: Khi hệ thống AI Engine bị quá tải hoặc gặp sự cố (CPU ≥ 85%, Memory ≥ 90%, Connections ≥ 450, Queue ≥ 10k), cơ chế **Fail-Open Fallback** (Lambda độc lập) sẽ kích hoạt trạng thái dự phòng và gửi cảnh báo tới quản trị viên (SRE/Ops) qua SNS/Slack. Để đối phó với nguy cơ kẻ tấn công cố tình làm giả mạo chỉ số hoặc quá tải hệ thống để ép hệ thống rơi vào trạng thái Fail-Open (kiểu tấn công Service Degradation Tampering), luồng cảnh báo này yêu cầu xác thực phê duyệt thủ công (Manual Approve Gate) bởi Ops trên dashboard Managed Grafana trước khi bất kỳ hành động thay đổi hạ tầng nào được thực thi.
