...
Tài liệu này mô tả cách cấp phát và phát hành platform TF4 Foresight Lens trên AWS. Kiến trúc runtime là source of truth: Producer/Mock Service → ALB/API → Ingest Service (ECS Fargate) → Amazon SQS Standard + DLQ → Ingest Worker (ECS Fargate) → Amazon Timestream → EventBridge Scheduler → AI Serving (ECS Fargate). CI/CD chỉ triển khai kiến trúc này, không thay đổi data flow.

Thiết kế triển khai này **không dùng Kinesis Data Streams hoặc Kinesis Firehose**. SQS là buffer chính của ingestion path; Ingest Worker chịu trách nhiệm poll, batch, retry và ghi dữ liệu vào Timestream.
...
```text
PR opened
  → Detect service source
  → Build container images nếu service có Dockerfile
...
Staging deployment chạy sau khi thay đổi được merge vào `develop`. Production/demo deployment chỉ chạy bằng manual dispatch, bắt buộc nhập full Git SHA đã pass staging và deploy bằng image digest đã tồn tại trong ECR. Vì vậy production không deploy image mutable tag và không tự build lại artifact.

### 2.2 GitHub Actions workflows

| Workflow | Trigger | Trách nhiệm | Quality gate |
|---|---|---|---|
| `build-test.yml` | PR; push `develop`, `main`; manual | Detect service source; chạy test Node/Python/Go nếu có; build Docker image nếu có Dockerfile | Build thành công, test pass |
| `security-scan.yml` | PR; push `develop`, `main`; manual | Gitleaks scan working tree; Trivy filesystem scan; Checkov scan Terraform | Không có secret; không có HIGH/CRITICAL CVE chưa được chấp nhận; Checkov pass hoặc có exception rõ |
| `terraform-plan.yml` | PR vào `develop`/`main`; manual | `terraform fmt`, `init`, `validate`, `plan`; đăng plan summary vào GitHub Actions | Plan được review trước khi merge |
| `deploy-staging.yml` | Push/merge vào `develop`; manual | Assume AWS deploy role bằng OIDC; build/push image lên ECR; `terraform apply staging`; deploy ECS; chạy smoke test | Staging deploy success, smoke test pass |
| `deploy-prod.yml` | Manual dispatch | Bắt buộc nhập Git SHA đã pass staging và confirm string; yêu cầu GitHub Environment approval; resolve image digest từ ECR; `terraform apply prod`; deploy ECS; chạy smoke test | Staging gate pass, prod approval, deploy success, smoke test pass |
...
- `main`: production-ready configuration; production/demo chỉ deploy bằng manual dispatch với Git SHA đã pass staging.
...
Git không lưu hoặc triển khai Kinesis stream/firehose trong thiết kế này. Nếu tài liệu kiến trúc khác nhắc Kinesis, phần đó cần được cập nhật để khớp runtime source of truth dùng SQS.
...
Workflow theo lịch chạy hằng ngày hoặc manual:
...
Health gate của Ingest Service gồm task health và ALB target group health. Health gate của Ingest Worker gồm task health, SQS queue depth, DLQ message count và Timestream write error.
...
Record sai schema được reject tại Ingest Service hoặc log/quarantine theo Telemetry Contract. Record hợp lệ nhưng xử lý thất bại ở worker sẽ được retry theo SQS redrive policy và chuyển đến Dead Letter Queue (DLQ) khi vượt quá `maxReceiveCount`.
